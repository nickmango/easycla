...
func UpdatePullRequest(ctx context.Context, apiBaseURL string, installationID int64, pullRequestID int, owner, repo, repositoryID, latestSHA, repositoryType string, signed []*UserCommitSummary, missing []*UserCommitSummary, badge CLABadge) error {
	f := logrus.Fields{
	...
	// Update the comment
	body := assembleClaComment(apiBaseURL, repositoryType, installationID, repositoryID, pullRequestID, signed, missing, badge)
...
func assembleClaComment(apiBaseURL, repositoryType string, installationID int64, repositoryID string, pullRequestID int, signed, missing []*UserCommitSummary, badge CLABadge) string {
	var allSigned bool
	var noUserID bool
	...
	commentBadge := getCommentBadge(signURL, allSigned, noUserID, isApproved, badge)
...
func getCommentBadge(signURL string, allSigned, missingUserID, isApproved bool, badge CLABadge) string {
	var alt string
	badgeHyperLink := signURL
	var badgeURL string

	if allSigned {
		badgeURL = fmt.Sprintf("%s/cla-signed.svg", badge.CLALogoURL)
		badgeHyperLink = fmt.Sprintf("%s#/?version=2", badge.CLALandingPage)
		alt = "CLA Signed"
	} else if missingUserID {
		badgeURL = fmt.Sprintf("%s/cla-missing-id.svg", badge.CLALogoURL)
		alt = "CLA Missing ID"
	} else if isApproved {
		badgeURL = fmt.Sprintf("%s/cla-confirmation-needed.svg", badge.CLALogoURL)
		alt = "CLA Confirmation Needed"
	} else {
		badgeURL = fmt.Sprintf("%s/cla-not-signed.svg", badge.CLALogoURL)
		alt = "CLA Not Signed"
	}
